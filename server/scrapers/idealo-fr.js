...
async function searchIdealoFR(query) {
  let browser;
  try {
    console.log(`[IDEALO.FR] Arama başlatılıyor: "${query}"`);
    ...
    const searchUrl = `https://www.idealo.fr/prechcat.html?q=${encodeURIComponent(query)}`;
    console.log(`[IDEALO.FR] Arama sayfasına gidiliyor: ${searchUrl}`);
    ...
    console.log(`[IDEALO.FR] Ürün elementleri aranıyor...`);
      ...
      productElements.forEach((element) => {
        ...
        try {
          ...
          if (linkElement) {
            ...
            link = href.startsWith('http') ? href : 'https://www.idealo.fr' + href;
          }
    ...
    console.log(`[IDEALO.FR] ${products.length} ürün bulundu`);

    await browser.close();

    if (products.length === 0) {
      console.log(`[IDEALO.FR] Ürün bulunamadı`);
      throw new Error('Ürün bulunamadı');
    }

    console.log(`[IDEALO.FR] Arama tamamlandı, ${products.length} ürün döndürülüyor`);
    return products;
  } catch (error) {
    if (browser) {
      await browser.close();
