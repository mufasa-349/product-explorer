...
async function setDeliveryZip(page, zip) {
  try {
    console.log(`[AMAZON.IT] Delivery address butonu aranıyor...`);
    ...
    for (const selector of locationSelectors) {
      ...
      if (el) {
        ...
        console.log(`[AMAZON.IT] Delivery address butonu tıklanıyor: ${selector}`);
        await el.click();
        clicked = true;
        break;
      }
    ...
    if (!foundInFirstPass && !clicked) {
      console.log(`[AMAZON.IT] 5 sn içinde bulunamadı, sayfa yenileniyor...`);
    ...
    if (!clicked) {
      console.log(`[AMAZON.IT] Delivery address butonu bulunamadı`);
      return false;
    }
    ...
    console.log(`[AMAZON.IT] Zip code input'u bekleniyor...`);
    await page.waitForSelector(zipInputSelector, { timeout: 10000 });
    const zipInput = await page.$(zipInputSelector);

    if (!zipInput) {
      console.log(`[AMAZON.IT] Zip input bulunamadı`);
      return false;
    }

    console.log(`[AMAZON.IT] Zip code input'una ${zip} giriliyor...`);
    await zipInput.click({ clickCount: 3 });
    await page.keyboard.press('Backspace');
    await zipInput.type(zip, { delay: 80 });
    await new Promise(resolve => setTimeout(resolve, 800));

    console.log(`[AMAZON.IT] Apply butonu tıklanıyor...`);
    const applyBtn = await page.$(applyButtonSelector);
    if (applyBtn) {
      await applyBtn.click();
    } else {
      console.log(`[AMAZON.IT] Apply butonu bulunamadı`);
      return false;
    }

    await new Promise(resolve => setTimeout(resolve, 2000));

    console.log(`[AMAZON.IT] Continue/Close adımı deneniyor...`);
    let closed = false;
    try {
      await page.waitForSelector(continueButtonSelector, { timeout: 6000 });
      const continueBtn = await page.$(continueButtonSelector);
      if (continueBtn) {
        console.log(`[AMAZON.IT] Continue butonu tıklanıyor...`);
        await continueBtn.click();
        closed = true;
        await new Promise(resolve => setTimeout(resolve, 1500));
      }
    } catch (e) {
      console.log(`[AMAZON.IT] Continue butonu görünmedi, alternatif kapanış deneniyor...`);
    }
    ...
    if (!closed) {
      try {
        ...
        if (closeBtn) {
          console.log(`[AMAZON.IT] Pop-up X butonu tıklanıyor...`);
    ...
    if (!closed) {
      console.log(`[AMAZON.IT] Pop-up kapatmak için ESC deneniyor...`);
      await page.keyboard.press('Escape');
      await new Promise(resolve => setTimeout(resolve, 1200));
    }

    console.log(`[AMAZON.IT] Delivery address ayarlama denemesi tamamlandı (${zip})`);
    return true;
  } catch (e) {
    console.log(`[AMAZON.IT] Delivery address ayarlama hatası: ${e.message}`);
    return false;
  }
}

async function searchAmazonIT(query) {
  let browser;
  try {
    console.log(`[AMAZON.IT] Arama başlatılıyor: "${query}"`);
    ...
    const searchUrl = `https://www.amazon.it/s?k=${encodeURIComponent(query)}`;
    console.log(`[AMAZON.IT] Arama sayfasına gidiliyor: ${searchUrl}`);
    ...
    await setDeliveryZip(page, '40120');

    try {
      await page.waitForSelector('[data-component-type="s-search-result"]', { timeout: 10000 });
      console.log(`[AMAZON.IT] Ürün elementleri yüklendi`);
    } catch (e) {
      console.log(`[AMAZON.IT] Ürün elementleri için selector bulunamadı, devam ediliyor...`);
    }

    await new Promise(resolve => setTimeout(resolve, 500)); // 2000 -> 500 ms

    console.log(`[AMAZON.IT] Ürün elementleri aranıyor...`);
    ...
    console.log(`[AMAZON.IT] ${products.length} eşleşen ürün bulundu`);

    await browser.close();

    if (products.length === 0) {
      console.log(`[AMAZON.IT] Ürün bulunamadı`);
      throw new Error('Ürün bulunamadı');
    }

    console.log(`[AMAZON.IT] Arama tamamlandı, ${products.length} ürün döndürülüyor`);
