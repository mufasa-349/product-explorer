...
async function setDeliveryZip(page, zip) {
  try {
    console.log(`[AMAZON.ES] Delivery address butonu aranıyor...`);
    ...
    for (const selector of locationSelectors) {
      ...
      if (el) {
        ...
        console.log(`[AMAZON.ES] Delivery address butonu tıklanıyor: ${selector}`);
        await el.click();
        clicked = true;
        break;
      }
    ...
    if (!foundInFirstPass && !clicked) {
      console.log(`[AMAZON.ES] 5 sn içinde bulunamadı, sayfa yenileniyor...`);
    ...
    if (!clicked) {
      console.log(`[AMAZON.ES] Delivery address butonu bulunamadı`);
      return false;
    }
    ...
    console.log(`[AMAZON.ES] Zip code input'u bekleniyor...`);
    await page.waitForSelector(zipInputSelector, { timeout: 10000 });
    const zipInput = await page.$(zipInputSelector);

    if (!zipInput) {
      console.log(`[AMAZON.ES] Zip input bulunamadı`);
      return false;
    }

    console.log(`[AMAZON.ES] Zip code input'una ${zip} giriliyor...`);
    await zipInput.click({ clickCount: 3 });
    await page.keyboard.press('Backspace');
    await zipInput.type(zip, { delay: 80 });
    await new Promise(resolve => setTimeout(resolve, 800));

    console.log(`[AMAZON.ES] Apply butonu tıklanıyor...`);
    const applyBtn = await page.$(applyButtonSelector);
    if (applyBtn) {
      await applyBtn.click();
    } else {
      console.log(`[AMAZON.ES] Apply butonu bulunamadı`);
      return false;
    }

    await new Promise(resolve => setTimeout(resolve, 2000));

    console.log(`[AMAZON.ES] Continue/Close adımı deneniyor...`);
    let closed = false;
    try {
      await page.waitForSelector(continueButtonSelector, { timeout: 6000 });
      const continueBtn = await page.$(continueButtonSelector);
      if (continueBtn) {
        console.log(`[AMAZON.ES] Continue butonu tıklanıyor...`);
        await continueBtn.click();
        closed = true;
        await new Promise(resolve => setTimeout(resolve, 1500));
      }
    } catch (e) {
      console.log(`[AMAZON.ES] Continue butonu görünmedi, alternatif kapanış deneniyor...`);
    }
    ...
    if (!closed) {
      try {
        ...
        if (closeBtn) {
          console.log(`[AMAZON.ES] Pop-up X butonu tıklanıyor...`);
    ...
    if (!closed) {
      console.log(`[AMAZON.ES] Pop-up kapatmak için ESC deneniyor...`);
      await page.keyboard.press('Escape');
      await new Promise(resolve => setTimeout(resolve, 1200));
    }

    console.log(`[AMAZON.ES] Delivery address ayarlama denemesi tamamlandı (${zip})`);
    return true;
  } catch (e) {
    console.log(`[AMAZON.ES] Delivery address ayarlama hatası: ${e.message}`);
    return false;
  }
}

async function searchAmazonES(query) {
  let browser;
  try {
    console.log(`[AMAZON.ES] Arama başlatılıyor: "${query}"`);
    ...
    console.log(`[AMAZON.ES] Arama sayfasına gidiliyor: ${searchUrl}`);
    ...
    await setDeliveryZip(page, '08001');

    try {
      await page.waitForSelector('[data-component-type="s-search-result"]', { timeout: 10000 });
      console.log(`[AMAZON.ES] Ürün elementleri yüklendi`);
    } catch (e) {
      console.log(`[AMAZON.ES] Ürün elementleri için selector bulunamadı, devam ediliyor...`);
    }

    await new Promise(resolve => setTimeout(resolve, 500)); // 2000 -> 500 ms

    console.log(`[AMAZON.ES] Ürün elementleri aranıyor...`);
      ...
      productElements.forEach((element) => {
        ...
        try {
          ...
          if (linkElement) {
            ...
            link = href.startsWith('http') ? href : 'https://www.amazon.es' + href;
          }
    ...
    console.log(`[AMAZON.ES] ${products.length} eşleşen ürün bulundu`);

    await browser.close();

    if (products.length === 0) {
      console.log(`[AMAZON.ES] Ürün bulunamadı`);
      throw new Error('Ürün bulunamadı');
    }

    console.log(`[AMAZON.ES] Arama tamamlandı, ${products.length} ürün döndürülüyor`);
