...
async function setDeliveryZip(page, zip) {
  try {
    ...
    if (!foundInFirstPass && !clicked) {
      console.log(`[AMAZON.DE] 5 sn içinde bulunamadı, sayfa yenileniyor...`);
    ...
    const products = await page.evaluate((searchQuery) => {
      ...
      productElements.forEach((element) => {
        ...
        try {
          ...
          const isSponsored = Boolean(sponsoredElement) || (element.textContent || '').toLowerCase().includes('gesponsert');
